...
import { useActiveLayerContext, useCanvasContext, useTextEditorContext } from '@/store'

export const TextEditor = () => {
  const { renderer, camera } = useCanvasContext()
  const { activeLayer } = useActiveLayerContext()
  const { isLayerEditable, text, textAlign, fontSize, bold, italic, underline, ...actions } =
  ...
  useEffect(() => {
    window.addEventListener(
      CustomEvents.DOUBLE_CLICK,
      handleDoubleClick as unknown as EventListener,
    )
    return () =>
