import { type Color, Tools } from '@/shared/interfaces'

export const DEFAULT_TOOL = Tools.SELECT

export const enum Colors {
  STICKER_YELLOW = '#ffd670',
  STICKER_ORANGE = '#ff9770',
  STICKER_GREEN = '#e9ff70',
  STICKER_DARK_GREEN = '#6ac5ab',
  STICKER_PURPLE = '#6965db',
  STICKER_DARK_PURPLE = '#5753d0',
  STICKER_BLUE = '#70d6ff',
  STICKER_PINK = '#ff70a6',
  TRANSPARENT = 'transparent',
}

export const COLOR_LIST: Color[] = [
  { value: '#70d6ff', label: '#70d6ff' },
  { value: '#f62a5c', label: '#f62a5c' },
  { value: '#ff70a6', label: '#ff70a6' },
  { value: '#e0bdfa', label: '#e0bdfa' },
  { value: '#fdc4e7', label: '#fdc4e7' },
  { value: '#ff9770', label: '#ff9770' },
  { value: '#ffd670', label: '#ffd670' },
  { value: '#e9ff70', label: '#e9ff70' },
  { value: '#6ac5ab', label: '#6ac5ab' },
]

export const DEFAULT_CURSOR = 'default'

const handCursor = new Image()

handCursor.src =
  'data:image/png;base64,'

export const CURSORS = {
  [Tools.HAND]: `-webkit-image-set(url("${handCursor.src}") 2x), auto`,
}
