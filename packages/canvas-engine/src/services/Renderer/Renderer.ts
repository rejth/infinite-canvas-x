...
import { Colors, TextAlign, TextDecoration } from '../../interfaces'
import { BezierCurve, geometry, SplineArcLengthMap } from '../../math'
import { ProxyCanvasRenderingContext2D, RenderMode } from '../RenderManager'
import { SPATIAL_TILE_SIZE } from '../SpatialTileIndex'
...
export class Renderer {
  ...
  getTransformedArea(): RectDimension {
    ...
    return {
      ...
      width:
        window.innerWidth * inverseScale + Math.abs(transformMatrix.translationX) * inverseScale,
      height:
        window.innerHeight * inverseScale + Math.abs(transformMatrix.translationY) * inverseScale,
    }
  }
