...
import type { DoubleClickCustomEvent } from '../../interfaces'
import { isCanvasRect } from '../../lib'
...
export class Camera {
  ...
  private currentTransformedPosition: Point = new Point(0, 0)
  ...
  moveCanvas(e: WheelEvent) {
    this.currentPosition = new Point(
      this.currentPosition.x + e.deltaX * -1,
      this.currentPosition.y + e.deltaY * -1,
    )
  ...
  zoomCanvas(e: WheelEvent) {
    ...
    if (nextZoomPercentage <= ZOOM_MAX && nextZoomPercentage >= ZOOM_MIN) {
      ...
      this.renderer.translate(
        -this.currentTransformedPosition.x,
        -this.currentTransformedPosition.y,
      )
      isZoomed = true
    }
  ...
  zoomCanvasWithStep(
    ...
    const nextZoomPercentage =
      step > 0 ? Math.min(zoomPercentage + step, edge) : Math.max(zoomPercentage + step, edge)
