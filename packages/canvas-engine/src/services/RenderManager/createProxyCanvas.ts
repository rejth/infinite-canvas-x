import { DEFAULT_CANVAS_SCALE } from '../../constants'
import { Colors, RectDimension } from '../../interfaces'
...
export function createProxyCanvas(
  ...
  const backgroundContext = backgroundCanvas.getContext(
    '2d',
  ) as unknown as ProxyCanvasRenderingContext2D
  ...
  return new Proxy(mainContext as unknown as ProxyCanvasRenderingContext2D, {
    get(target, property: keyof ProxyCanvasRenderingContext2D) {
      ...
      return (...args: unknown[]) => {
        ...
        if (
          ['scale', 'translate', 'setTransform'].includes(property) &&
          currentMode !== RenderMode.TEXT
        ) {
