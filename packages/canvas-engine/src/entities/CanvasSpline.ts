import { DEFAULT_SCALE, SMALL_PADDING } from '../constants'
import { Colors, Point } from '../interfaces'
import { BezierCurve, MBR, Vector } from '../math'

import { BaseCanvasEntity } from './BaseCanvasEntity'
import { CanvasRect } from './CanvasRect'
import { BaseDrawOptions, CanvasEntitySubtype, CanvasEntityType } from './interfaces'
  ...
  private dragControlPoint = (cp: Point, cpIndex: number) => {
    ...
    if (splineIndex % 3 === 0) {
      ...
      if (m >= 0 && m < this.spline.length) {
        ...
        this.spline[m] = this.vAdd(
          this.spline[k],
          this.vSub(this.spline[k], this.spline[splineIndex]),
        )
      }
    }
